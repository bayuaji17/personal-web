...
export default function ProfileSection() {
  ...
  const fallbackSrc = "/profile_anime.webp";

  const src = theme === "light" ? "/profile_anime.webp" : "/profile_wpap.webp";
  return (
    <Card
      role="region"
      aria-label="Profile section"
      className="items-center md:flex-row md:justify-evenly md:items-center p-4"
    >
      <Avatar className="size-40 shadow-sm border">
        {isMounted ? (
          <Image
            src={src}
            alt="Profile picture"
      ...
      <div
        className="flex flex-col justify-center items-center md:items-baseline space-y-3"
        role="group"
        aria-label="Profile details"
      >
        <h1 className="font-black text-3xl text-center">Bayu Aji Nugroho</h1>
        <p className="text-2xl font-semibold -tracking-tighter">
          Web Developer
        </p>
        <div className="flex gap-3" role="group" aria-label="Social links">
          <Button asChild className="rounded-full size-12 text-popover">
            <Link
              href={"mailto:nugroho.bayuaji17@gmail.com?subject=Job%20Offer"}
